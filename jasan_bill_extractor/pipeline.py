...
pipeline.py
---------------------------------
Phase 2~3 파이프라인 본체 (spec.md §6): 전처리 -> 추출 -> 검증
(ArithmeticValidator + CrossFieldValidator) -> 사업장 매칭(SiteMatcher) -> 이력
이상탐지(HistoryAnomalyDetector)를 배치로 실행한다.

CLI(scripts/run_pipeline.py)와 GUI(gui/app.py)가 공유하는 핵심 로직이다. 반드시
jasan_bill_extractor 패키지 안의 정식 모듈로 둬야 한다 - PyInstaller로 패키징할 때
scripts/ 폴더의 느슨한 스크립트를 sys.path 조작 + bare import로 불러오면 정적
분석(modulegraph)이 그 임포트를 추적하지 못해 번들에서 빠지고
"ModuleNotFoundError: No module named 'run_pipeline'"가 난다 (Phase 5 실배포 중 발견됨).
"""
...
from jasan_bill_extractor.preprocess.image_prep import prep_file
from jasan_bill_extractor.extract.vision_client import (
    DEFAULT_MODEL,
    extract_from_image,
    VisionExtractionError,
)
from jasan_bill_extractor.validate.arithmetic import validate_due_total, validate_overdue_total
from jasan_bill_extractor.validate.cross_field import validate_regions
from jasan_bill_extractor.validate import history
from jasan_bill_extractor.match.site_master import load_site_master
from jasan_bill_extractor.match.site_matcher import SiteMatcher, MatchResult
...
def run_pipeline(
    ...
    """Phase 2~3 파이프라인 본체. GUI(gui.app)와 CLI(scripts/run_pipeline.py)가 공유하는
    핵심 로직.
